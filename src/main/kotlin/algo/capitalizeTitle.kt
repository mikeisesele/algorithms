...
//Explanation: Since all the words have a length of at least 3,
...
fun capitalizeTitle(title: String): String {
    val result = title.split(" ").map {
        if (it.length <= 2) {
            it.lowercase(Locale.getDefault())
        } else {
            it.first().uppercaseChar() + it.substring(1).lowercase(Locale.getDefault())
        }
    }
